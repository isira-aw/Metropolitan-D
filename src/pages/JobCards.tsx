...
import { Plus, Filter, Calendar, Clock, Users, Wrench, Settings } from 'lucide-react';
import { apiService } from '../services/api';
import { JobCardResponse, GeneratorResponse, EmployeeResponse, CreateJobCardRequest } from '../types/api';
...
export const JobCards: React.FC = () => {
  ...
  const handleCreateJob = async () => {
    try {
      const response = jobType === 'SERVICE' 
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex space-x-2">
            {['ALL', 'SERVICE', 'REPAIR'].map((type) => (
              <button
                key={type}
                onClick={() => setFilterType(type as 'ALL' | 'SERVICE' | 'REPAIR')}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {filteredJobCards.map((job) => (
          <div key={job.jobCardId} className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow">
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center space-x-3">
                <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${
                  ...
                  {job.jobType === 'SERVICE' ? (
                    <Settings className={`w-6 h-6 ${job.jobType === 'SERVICE' ? 'text-green-600' : 'text-orange-600'}`} />
                  ) : (
                    <Wrench className={`w-6 h-6 ${job.jobType === 'SERVICE' ? 'text-green-600' : 'text-orange-600'}`} />
                  )}
                </div>
                <div>
                  <h3 className="font-semibold text-slate-900">{job.generator.name}</h3>
                  <p className="text-sm text-slate-500">{job.generator.capacity}</p>
                </div>
              </div>
            ...
            <div className="border-t border-slate-200 pt-4">
              <p className="text-sm font-medium text-slate-700 mb-2">Assigned Employees:</p>
      ...
      >
        <div className="space-y-6">
          {/* Job Type Selection */}
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-3">Job Type</label>
          ...
          {/* Generator Selection */}
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">Generator</label>
            <select
              value={formData.generatorId}
              onChange={(e) => setFormData(prev => ({ ...prev, generatorId: e.target.value }))}
              className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            >
              <option value="">Select a generator</option>
              {generators.map((generator) => (
                <option key={generator.generatorId} value={generator.generatorId}>
          ...
          <div>
            ...
            <div className="max-h-48 overflow-y-auto border border-slate-300 rounded-lg p-3 space-y-2">
              {employees.map((employee) => (
                <label key={employee.email} className="flex items-center space-x-3 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={formData.employeeEmails.includes(employee.email)}
                    onChange={() => handleEmployeeToggle(employee.email)}
                    disabled={!formData.employeeEmails.includes(employee.email) && formData.employeeEmails.length >= 5}
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            <button
              ...
              disabled={!formData.generatorId || !formData.date || !formData.estimatedTime || formData.employeeEmails.length === 0}
