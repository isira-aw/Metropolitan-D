...
export const Dashboard: React.FC = () => {
  ...
  const generateReportTable = async (): Promise<void> => {
    ...
    try {
      console.log('Generating report for:', { selectedEmployee, startDate, endDate });
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <div className="flex items-center mb-6">
          ...
          <h2 className="text-xl font-semibold text-slate-900">Employee Work Report</h2>
        </div>
              ...
              <table className="min-w-full">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Generator Names</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">First Location</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Last Location</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Working Hours</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Morning OT</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Evening OT</th>
                  </tr>
                </thead>
                  ...
                  {reportData.map((day, index) => (
                    <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-slate-50'}>
                      ...
                      <td className="px-4 py-4 text-sm text-slate-900">
                        <span className="truncate max-w-xs block" title={day.generatorNames}>
                          {day.generatorNames || 'N/A'}
                        </span>
                      </td>
                      <td className="px-4 py-4 text-sm text-slate-900">
                        <span className="truncate max-w-xs block" title={day.firstActionLocation}>
                          {day.firstActionLocation || 'N/A'}
                        </span>
                      </td>
                      <td className="px-4 py-4 text-sm text-slate-900">
                        <span className="truncate max-w-xs block" title={day.lastActionLocation}>
        ...
        {/* No Data Message */}
        {reportData.length === 0 && selectedEmployee && startDate && endDate && !reportLoading && !reportError && (
          <div className="text-center text-slate-500 py-8">
            <Search className="h-12 w-12 mx-auto mb-4 text-slate-300" />
            <p className="text-lg">No data found</p>
            <p className="text-sm">No work data found for the selected date range.</p>
            <button 
              onClick={() => console.log('Debug info:', { selectedEmployee, startDate, endDate, reportData })}
        ...
        <div className="mt-4 p-3 bg-gray-50 rounded text-xs text-gray-600">
          ...
          End: {endDate || 'None'} | 
          Data Count: {reportData.length} | 
          Loading: {reportLoading ? 'Yes' : 'No'} | 
